      ...
      {backendWalletAddress ? (
        backendHexStakeLoading ? (
          ...
          <div style={{ display: 'grid', gap: 4 }}>
            ...
            {backendHexStakeResponse.data.positions.length ? (
              <ul style={{ margin: 0, paddingLeft: 18 }}>
                {backendHexStakeResponse.data.positions
                  .filter((position) => position.stakeSource === 'native')
                  .map((position) => (
              ...
              <div>No native positions returned.</div>
            )}
          </div>
        ) : (
          <div>Backend HEX stakes unavailable: {backendHexStakeResponse?.error?.message ?? 'unknown error'}</div>
        )
      ) : (
