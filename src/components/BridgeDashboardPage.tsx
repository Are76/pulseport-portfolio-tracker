...
// ── Section with pulsechainstats-style colored left border ────────────────────
...
export default function BridgeDashboardPage() {
  ...
  return (
    <div style={{ maxWidth: 960, margin: '0 auto', paddingBottom: 48 }}>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12, marginBottom: 22 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
            ...
            <div>
              <h1 style={{ fontSize: 26, fontWeight: 800, color: 'var(--fg)', margin: 0, letterSpacing: '-0.5px' }}>Bridge Hub</h1>
              <p style={{ fontSize: 13, color: 'var(--fg-muted)', margin: '3px 0 0' }}>Bridges, on-ramps & cross-chain routes to PulseChain</p>
            </div>
          </div>
      ...
      <Section title="Official PulseChain Bridge" icon={<Zap size={15} color="var(--accent)" />} color="var(--accent)" badge="Live TVL">
        ...
        <div style={{ marginTop: 12, fontSize: 11, color: 'var(--fg-subtle)', lineHeight: 1.6 }}>
          TVL = total supply of each bridged token × current market price. Supply from PulseChain scanner. Prices from CoinGecko.
        </div>
      </Section>

      {/* ── Hyperlane ─────────────────────────────────────────────────────────── */}
      <Section title="Hyperlane" icon={<ArrowUpDown size={15} color="#627EEA" />} color="#627EEA" badge="160+ Chains">
        <p style={{ fontSize: 13, color: 'var(--fg-muted)', lineHeight: 1.7, margin: '0 0 18px' }}>
      ...
      {/* ── Liberty Swap ──────────────────────────────────────────────────────── */}
      <Section title="Liberty Swap" icon={<Shield size={15} color="#a855f7" />} color="#a855f7" badge="Privacy Bridge">
        <p style={{ fontSize: 13, color: 'var(--fg-muted)', lineHeight: 1.7, margin: '0 0 18px' }}>
      ...
      {/* ── On-Ramps ──────────────────────────────────────────────────────────── */}
      <Section title="Fiat & Crypto On-Ramps" icon={<CreditCard size={15} color="#f59e0b" />} color="#f59e0b" badge="Direct to PulseChain">
        <p style={{ fontSize: 13, color: 'var(--fg-muted)', lineHeight: 1.7, margin: '0 0 18px' }}>
      ...
      {/* ── Bridge Comparison ─────────────────────────────────────────────────── */}
      <Section title="Bridge Comparison" icon={<ArrowLeftRight size={15} color="var(--fg-muted)" />} color="var(--fg-muted)">
        <div style={{ overflowX: 'auto' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
            <thead>
              <tr style={{ background: 'var(--bg-elevated)' }}>
                {['Bridge', 'Type', 'Fee', 'Speed', 'Privacy', 'TVL'].map(h => (
                  <th key={h} style={{ padding: '8px 12px', color: 'var(--fg-muted)', fontWeight: 700, fontSize: 10, textTransform: 'uppercase', letterSpacing: '.5px', textAlign: 'left', borderBottom: '1px solid var(--border)', whiteSpace: 'nowrap' }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {[
                ...
                <tr key={row.name} style={{ background: i % 2 === 0 ? 'transparent' : 'var(--bg-elevated)', borderBottom: '1px solid var(--border)' }}>
                  <td style={{ padding: '11px 12px', fontWeight: 700, color: row.color }}>{row.name}</td>
                  <td style={{ padding: '11px 12px', color: 'var(--fg-muted)' }}>{row.type}</td>
                  <td style={{ padding: '11px 12px', color: 'var(--fg)', fontFamily: 'JetBrains Mono, monospace' }}>{row.fee}</td>
                  <td style={{ padding: '11px 12px', color: 'var(--fg)' }}>{row.speed}</td>
                  <td style={{ padding: '11px 12px', color: 'var(--fg-muted)' }}>{row.privacy}</td>
                  <td style={{ padding: '11px 12px', fontWeight: 700, color: 'var(--fg)', fontFamily: 'JetBrains Mono, monospace' }}>{row.tvl}</td>
                </tr>
              ))}
            </tbody>
          </table>
