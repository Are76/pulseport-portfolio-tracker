import { getHexStakeDashboard, HexStakeServiceError } from '../../src/server/hex-stakes/hex-stake-service';
import type { HexStakeDashboardResponse } from '../../src/server/hex-stakes/hex-stake-types';
...
type ApiResponse = {
  ...
  json: (payload: HexStakeDashboardResponse) => void;
...
export default async function handler(req: ApiRequest, res: ApiResponse) {
  ...
  try {
    const data = await getHexStakeDashboard(walletAddress, chainId);
    return res.status(200).json({ ok: true, data, error: null });
  } catch (error) {
    if (error instanceof HexStakeServiceError) {
