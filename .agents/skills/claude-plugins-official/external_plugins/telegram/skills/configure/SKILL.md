...
Writes the bot token to `~/.Codex/channels/telegram/.env` and orients the
...
1. **Token** — check `~/.Codex/channels/telegram/.env` for
   `TELEGRAM_BOT_TOKEN`. Show set/not-set; if set, show first 10 chars masked
   (`123456789:...`).

2. **Access** — read `~/.Codex/channels/telegram/access.json` (missing file
...
2. `mkdir -p ~/.Codex/channels/telegram`
3. Read existing `.env` if present; update/add the `TELEGRAM_BOT_TOKEN=` line,
   preserve other keys. Write back, no quotes around the value.
4. `chmod 600 ~/.Codex/channels/telegram/.env` — the token is a credential.
